...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(in_files), 1):
       ...
       if in_files[idx][0] != ".":
           ...
           #Recall that each filename can be
           #          accessed by in_files[idx]
           file_pet=in_files[idx]
           #If files names has upper case convert the following to lower
         # If fil name has "_" in the name remove the following
           file_pet=file_pet.lower().split("_")
           for name in file_pet:
                pet_label+=name
           pet_label.strip()
